...
%%%% flux-based Mahalanobis matrix, using equation (0.10) in the notes %%%%
...
efcns = 5;       %number of eigenfunctions to keep
bta = 10^(-2);   %regularization parameter
M = eye(d);      %initial (square root of) Mahalanobis matrix
obs = @(x) x;    %observable of interest (MUST be full-state observable!)
...
for iter = 1:iters
    ...
    M = get_mahalanobis_matrix(V,Lam,efcns,Phi_x);
...
function M = get_mahalanobis_matrix(V,Lam,efcns,Phi_x)

disp('getting Mahalanobis matrix...')

%convert Lam to vector form, and take log
LogLam = log(diag(Lam));

%compute flux
F = (LogLam(1:efcns).'.*Phi_x(:,1:efcns))*V(:,1:efcns)'; 

%get M and regularize
M = F'*F; M = sqrtm(M); M = M/norm(M);
